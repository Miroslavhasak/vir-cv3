...
for s = 1:nScenarios
    ...
    Kp_best = pop(bestIdx, 1);
    Ki_best = pop(bestIdx, 2);
    Kd_best = pop(bestIdx, 3);
    [fitnes(s), wBest, yBest, uBest] = simulatePID(Kp_best, Ki_best, Kd_best, scenario1, simTime, dt, uMax, a1_val, b0_val);
    fprintf('Fitness (ISE) v scenári %i pre vyvinutý NC je %.2f\n', s, fitnes(s));
    fprintf('\nOptimálne Kp: %.4f, Ki: %.4f, Kd: %.4f\n', Kp_best, Ki_best, Kd_best);
    % *** ZMENA ZAČÍNA TU ***
    
    % 1. Vykreslenie Výstupu y(t)
    figure('Name',['PID Optimalizácia - Výstup Scenár ', num2str(s)]);
